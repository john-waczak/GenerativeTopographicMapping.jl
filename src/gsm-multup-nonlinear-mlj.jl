mutable struct GSMMultUpNonlinear<: MLJModelInterface.Unsupervised
    k::Int
    m::Int
    s::Float64
    Nv::Int
...
function GSMMultUpNonlinear(; k=10, m=5, s=1.0, Nv=3, λ=0.1, nepochs=100, niters=100, tol=1e-3, nconverged=4, rand_init=true, rng=123)
    model = GSMMultUpNonlinear(k, m, s, Nv, λ, nepochs, niters, tol, nconverged, rand_init, mk_rng(rng))
...
function MLJModelInterface.clean!(m::GSMMultUpNonlinear)
    warning =""
...
function MLJModelInterface.fit(m::GSMMultUpNonlinear, verbosity, Datatable)
    ...
    # 1. build the GTM
    gsm = GSMMultUpNonlinearBase(m.k, m.m, m.s, m.Nv, X; rand_init=m.rand_init, rng=m.rng)
...
MLJModelInterface.fitted_params(m::GSMMultUpNonlinear, fitresult) = (gsm=fitresult,)


function MLJModelInterface.predict(m::GSMMultUpNonlinear, fitresult, Data_new)
...
function MLJModelInterface.transform(m::GSMMultUpNonlinear, fitresult, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function predict_responsibility(m::MLJBase.Machine{GSMMultUpNonlinear, GSMMultUpNonlinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function data_reconstruction(m::MLJBase.Machine{GSMMultUpNonlinear, GSMMultUpNonlinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
    gsm = fitted_params(m)[:gsm]
