...
include("gtm-base.jl")
include("gsm-base-log.jl")
include("gsm-base-normal.jl")
...
MLJModelInterface.@mlj_model mutable struct GSMLog <: MLJModelInterface.Unsupervised
...
MLJModelInterface.@mlj_model mutable struct GSMNormal <: MLJModelInterface.Unsupervised
    k::Int = 10::(_ > 0)                              # nodes per edge
    m::Int = 5::(_ > 0)                               # rbf per edge
    Nv::Int = 3::(_ > 0)                              # n-vertices, i.e. n-endmembers
    s::Float64 = 1.0::(_ > 0)                         # rbf variance scale factor
...
function MLJModelInterface.fit(m::GSMLog, verbosity, Datatable)
    ...
    # 1. build the GTM
    gsm = GSMBaseLog(m.k, m.m, m.s, m.Nv, X; rand_init=m.rand_init)
...
function MLJModelInterface.fit(m::GSMNormal, verbosity, Datatable)
    # assume that X is a table
    X = MLJModelInterface.matrix(Datatable)
    ...
    # 1. build the GTM
    gsm = GSMBaseNormal(m.k, m.m, m.s, m.Nv, X; rand_init=m.rand_init)
...
MLJModelInterface.fitted_params(m::GSMNormal, fitresult) = (gsm=fitresult,)
...
function MLJModelInterface.predict(m::GSMNormal, fitresult, Data_new)
    # Return the mode index as a class label
    Xnew = MLJModelInterface.matrix(Data_new)
    n_nodes = binomial(m.k + m.Nv - 2, m.Nv - 1)
    labels = MLJModelInterface.categorical(1:n_nodes) # there are k^2 many SOM nodes
...
function MLJModelInterface.transform(m::GSMNormal, fitresult, Data_new)
    # return a table with the mean ξ₁ and ξ₂ for each record
    Xnew = MLJModelInterface.matrix(Data_new)
    ξmeans = DataMeans(fitresult, Xnew)
    names = [Symbol("ξ$(i)") for i ∈ 1:m.Nv]
...
function predict_responsibility(m::MLJBase.Machine{GSMNormal, GSMNormal, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
    gsm = fitted_params(m)[:gsm]
    return responsibility(gsm, Xnew)
end
...
MLJModelInterface.metadata_model(
    ...
    output_scitype  = MLJModelInterface.Table(MLJModelInterface.Continuous),
    supports_weights = false,
	  load_path    = "GenerativeTopographicMapping.GSMNormal"
)
...
"""
$(MLJModelInterface.doc_header(GSMNormal))

Generative Simplex Mapping based based on [Generative Topographic Mapping](https://direct.mit.edu/neco/article/10/1/215-234/6127),
  Neural Computation; Bishop, C.; (1998):
  \"GTM: The Generative Topographic Mapping\"
where the latent points are configured as a gridded n-simplex and the data space is given by a normal distribution.
...
- `gsm`: The `GenerativeSimplexMap` object fit by the `GSMNormal` model. Contains node coordinates, RBF means, RBF variance, weights, etc.
...
```
using MLJ
gsm = @load GSMNormal pkg=GenerativeTopographicMapping
model = gsm()
X, y = make_blob(100, 10; centers=5) # synthetic data
mach = machine(model, X) |> fit!
X̃ = transform(mach, X)
