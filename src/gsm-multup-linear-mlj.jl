mutable struct GSMMultUpLinear<: MLJModelInterface.Unsupervised
    k::Int
    Nv::Int
    λ::Float64
    nepochs::Int
...
function GSMMultUpLinear(; k=10, Nv=3, λ=0.1, nepochs=100, niters=100, tol=1e-3, nconverged=4, rand_init=true, rng=123)
    model = GSMMultUpLinear(k, Nv, λ, nepochs, niters, tol, nconverged, rand_init, mk_rng(rng))
...
function MLJModelInterface.clean!(m::GSMMultUpLinear)
    warning =""
...
function MLJModelInterface.fit(m::GSMMultUpLinear, verbosity, Datatable)
    ...
    # 1. build the GTM
    gsm = GSMMultUpLinearBase(m.k, m.Nv, X; rand_init=m.rand_init, rng=m.rng)
...
MLJModelInterface.fitted_params(m::GSMMultUpLinear, fitresult) = (gsm=fitresult,)


function MLJModelInterface.predict(m::GSMMultUpLinear, fitresult, Data_new)
...
function MLJModelInterface.transform(m::GSMMultUpLinear, fitresult, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function predict_responsibility(m::MLJBase.Machine{GSMMultUpLinear, GSMMultUpLinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function data_reconstruction(m::MLJBase.Machine{GSMMultUpLinear, GSMMultUpLinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
    gsm = fitted_params(m)[:gsm]
