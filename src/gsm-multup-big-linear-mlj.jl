mutable struct GSMMultUpBigLinear<: MLJModelInterface.Unsupervised
    n_nodes::Int
    Nv::Int
    λ::Float64
    nepochs::Int
...
function GSMMultUpBigLinear(; n_nodes=1000, Nv=3, λ=0.1, nepochs=100, niters=100, tol=1e-3, nconverged=4, rand_init=true, rng=123)
    model = GSMMultUpBigLinear(n_nodes, Nv, λ, nepochs, niters, tol, nconverged, rand_init, mk_rng(rng))
...
function MLJModelInterface.clean!(m::GSMMultUpBigLinear)
    warning =""
...
function MLJModelInterface.fit(m::GSMMultUpBigLinear, verbosity, Datatable)
    ...
    # 1. build the GTM
    gsm = GSMMultUpBigLinearBase(m.n_nodes, m.Nv, X; rand_init=m.rand_init, rng=m.rng)
...
MLJModelInterface.fitted_params(m::GSMMultUpBigLinear, fitresult) = (gsm=fitresult,)


function MLJModelInterface.predict(m::GSMMultUpBigLinear, fitresult, Data_new)
...
function MLJModelInterface.transform(m::GSMMultUpBigLinear, fitresult, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function predict_responsibility(m::MLJBase.Machine{GSMMultUpBigLinear, GSMMultUpBigLinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
...
function data_reconstruction(m::MLJBase.Machine{GSMMultUpBigLinear, GSMMultUpBigLinear, true}, Data_new)
    Xnew = MLJModelInterface.matrix(Data_new)
    gsm = fitted_params(m)[:gsm]
