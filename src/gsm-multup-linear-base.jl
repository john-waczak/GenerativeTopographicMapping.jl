mutable struct GSMMultUpLinearBase{T1 <: AbstractArray, T2 <: AbstractArray, T3 <: AbstractArray, T4 <: AbstractArray, T5 <: AbstractArray, T6 <: AbstractArray, T7 <: AbstractArray}
...
function GSMMultUpLinearBase(k, Nᵥ, X; rand_init=true, rng=mk_rng(123))
...
function fit!(gsm::GSMMultUpLinearBase, X; λ = 0.1, nepochs=100, tol=1e-3, nconverged=5, verbose=false, n_steps=25)
...
function DataMeans(gsm::GSMMultUpLinearBase, X)
...
function DataModes(gsm::GSMMultUpLinearBase, X)
    mul!(gsm.Ψ, gsm.W, gsm.Φ')
    Δ² = pairwise(sqeuclidean, gsm.Ψ, X', dims=2)
    Δ² .*= -(1/(2*gsm.β⁻¹))
...
function class_labels(gsm::GSMMultUpLinearBase, X)
    mul!(gsm.Ψ, gsm.W, gsm.Φ')
    Δ² = pairwise(sqeuclidean, gsm.Ψ, X', dims=2)
    Δ² .*= -(1/(2*gsm.β⁻¹))
...
function responsibility(gsm::GSMMultUpLinearBase, X)
    mul!(gsm.Ψ, gsm.W, gsm.Φ')
    Δ² = pairwise(sqeuclidean, gsm.Ψ, X', dims=2)
    Δ² .*= -(1/(2*gsm.β⁻¹))
